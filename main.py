...
if button==True:
    ...
    #st.pyplot(pltp)
    st.plotly_chart(pltp,use_container_width=True,height=600)
    st.header('日経平均との累積リターンの比較')
    pltn,nikkeidf=nikkei.nikkei(stock_data,s,e)
    st.plotly_chart(pltn,use_container_width=True,height=600)
    st.header("市場β値")
    pltb,beta1=beta.beta(stock_data,nikkeidf,s,e)
    st.text("市場β値："+beta1)
    st.subheader("日経平均株価との価格推移比較（正規化）")
    st.plotly_chart(pltb,use_container_width=True,height=600)
    st.header("購入株数")
